...
class EBranchformerEncoder(AbsEncoder):
    ...
    def __init__(
        ...
        elif input_layer=="fused":
            self.embed= Conv2dSubsamplingFUSED(
                input_size,
                output_size,
                dropout_rate,
                pos_enc_class(output_size, positional_dropout_rate, max_pos_emb_len),
    ...
    def forward(
        ...
        if (
            ...
            or isinstance(self.embed, Conv2dSubsamplingFUSED)
        ):
...
class MidLevelFusion(AbsEncoder):
    """MidLevelFusion encoder module."""
    ...
    def __init__(
        ...
        if fusion_weights is None:
            ...
            self.fusion_weights = nn.Parameter(torch.tensor(fusion_weights, dtype=torch.float))

    def output_size(self) -> int:
        return self.encoders[0].output_size()

    def forward(
        self,
        xs_pad_list: List[torch.Tensor],
        ilens_list: List[torch.Tensor],
        prev_states: Optional[List[torch.Tensor]] = None,
        ctc: Optional[CTC] = None,
        max_layer: Optional[int] = None,
    ) -> Tuple[torch.Tensor, torch.Tensor, Optional[torch.Tensor]]:
        assert len(xs_pad_list) == 4
        #print(ilens_list) #38?
        #assert len(ilens_list) == 4;

        encoder_outputs = []
        output_lengths = []

        for i in range(4):
            xs_pad, ilens = xs_pad_list[i], ilens_list
            encoder_output, olens, _ = self.encoders[i](xs_pad, ilens_list, prev_states, ctc, max_layer)
            encoder_outputs.append(encoder_output)
            output_lengths.append(olens)

        # Weighted sum of encoder outputs
        fused_output = sum(w * enc_out for w, enc_out in zip(self.fusion_weights, encoder_outputs))
        # Normalize the lengths (they should all be the same)
        fused_lengths = output_lengths[0]  # Assuming all lengths are the same

        return fused_output, fused_lengths, None
